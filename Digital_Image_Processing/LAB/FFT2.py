...
def main():
    ...
    centered_fft_img = np.fft.fftshift(fft_img)

    magnitude_spectrum = np.log(np.abs(fft_img))
    centered_magnitude_spectrum = np.log(np.abs(centered_fft_img))
    ...
    filter_ft_img = centered_fft_img * filter
